...
import { FormTestimonyRequest } from "@/types/TestimonyType";
...
const FormCreateTestimony = ({ errorMessage }: { errorMessage?: string }) => {
  const { data, setData, post, processing, errors, reset } =
    useForm<FormTestimonyRequest>({
      testimoni_name: "",
      satisfaction: 5,
      expression: "",
      avatar_testimoni: null,
    });

  const { resetErrorCount } = useErrorNotifier(
    "Gagal Tambah Testimoni",
    errorMessage,
  );
  ...
  const onSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route("testimony.create.request"), {
      onError: (errors) => {
              ...
              {data.avatar_testimoni && (
                <>
                  ...
                  <div className="mt-4 flex w-full flex-col items-center justify-center">
                    <img
                      src={URL.createObjectURL(data.avatar_testimoni as File)}
...
export default FormCreateTestimony;
