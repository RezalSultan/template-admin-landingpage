...
import { DataGallerySchema, FormGalleryRequest } from "@/types/GalleryType";
...
const FormEditGallery = ({
  ...
  const { data, setData, post, processing, errors, reset } =
    useForm<FormGalleryRequest>({
      name_url: dataGallery.name_url,
      title: dataGallery.title,
      description: dataGallery.description ?? "",
      event_date: dataGallery.event_date ?? null,
    });
  ...
  const onSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route("gallery.edit.request", { id: dataGallery.id }), {
      onError: (errors) => {
                ...
                <Popover>
                  ...
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar
                      ...
                      disabled={(event_date) => {
                        return event_date < new Date("1900-01-01");
                      }}
                      initialFocus
                    />
                  </PopoverContent>
                  ...
                  <div className="mt-4 flex w-full flex-col items-center justify-center">
                    <img
                      src={
                        typeof data.name_url === "string"
                          ? `/${data.name_url}`
                          : URL.createObjectURL(data.name_url as File)
                      }
