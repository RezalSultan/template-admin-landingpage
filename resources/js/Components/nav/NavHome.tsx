...
const NavbarHome = () => {
  return (
    <header
      className={`fixed inset-0 z-[150] h-20 w-full bg-primary shadow-lg transition-all duration-300 ease-in-out`}
    >
      <div className="flex h-20 items-center justify-between gap-10 px-5 text-primary-foreground xl:container sm:px-10 lg:px-20 xl:mx-auto">
        <Link href="/">
          <p className="text-2xl">LOGO</p>
        </Link>
        <nav className="font-display hidden h-20 items-center justify-center text-lg lg:flex">
          <ul className="items-start justify-start text-center text-base font-normal lg:flex lg:gap-8 lg:text-lg">
            {routes.map((route) => (
              <NavbarHomeItem
                key={route.href}
                label={route.label}
