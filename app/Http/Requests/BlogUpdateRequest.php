...
class BlogUpdateRequest extends FormRequest
{
  ...
  public function rules(): array
  {
    ...
    if ($this->hasFile('cover_image') && $this->file('cover_image')->isValid()) {
      $rules['cover_image'] = 'nullable|file|mimes:jpg,jpeg,png|max:5120';
    } elseif (is_string($this->input('cover_image'))) {
      $rules['cover_image'] = 'nullable';
    }

    foreach ($this->input('sections', []) as $index => $section) {
      if ($section['type_view_content'] === 'IMAGE') {
        if ($this->hasFile("sections.$index.url_asset") && $this->file("sections.$index.url_asset")->isValid()) {
          $rules["sections.$index.url_asset"] = 'nullable|file|mimes:jpg,jpeg,png|max:5120';
        } else {
          $rules["sections.$index.url_asset"] = 'nullable';
        }
      } else {
        $rules["sections.$index.url_asset"] = 'nullable|url';
      }
    }
